...
# 嘉宾查询接口
def get_guest_list(requset):
    eid = requset.GET.get('eid', )          # 关联发布会 id
    phone = requset.GET.get('phone', )      # 关联手机号
    if eid == '':
...
def user_sign(request):
    eid = request.POST.get('eud', '')   # 发布会 id
    ...
    result = Event.objects.get(id=eid).status
    if not result:
        return JsonResponse({'suatus': 10023, 'message': 'event status is not available'})
    event_time = Event.objects.get(id=eid).start_time       # 发布会时间
    etime = str(event_time).split('.')[0]
    timeArray = time.strptime(etime, '%Y-%m-%d %H:%M:%S')
    e_time = int(time.mktime(timeArray))
    ...
    if result:
        return JsonResponse({'status': 10027, 'message': 'user hass ignin'})
    else:
